...
def random_crop(image, label, crop_height, crop_width, v_aoe=AOI_PERC):
    if (image.shape[0] != label.shape[0]) or (image.shape[1] != label.shape[1]):
        raise Exception('Image and label must have the same dimensions!')
        
    if (crop_width <= image.shape[1]) and (crop_height <= image.shape[0]):
        v_non_aoe_px = int(crop_height * (1.0 - v_aoe))
        x = random.randint(0, image.shape[1]-crop_width)
        y = random.randint(0, image.shape[0]-crop_height-v_non_aoe_px)
...
def gen_batch_function(data_folder, rgb_dir, seg_dir, args):
    """
    Generate function to create batches of training data
    """
    image_shape = (args.crop_height, args.crop_width)
